...
function markerEvent() {
    ...
    //}
    const icao = targetMarker.innerText[0] + targetMarker.innerText[1] + targetMarker.innerText[2] + targetMarker.innerText[3];
...
//flight code to next place
    ...
    function sellOrLose() {
        ...
        async function getJson() {
            try {
                const results = await fetchJson('http://127.0.0.1:3000/sell/' + selectValue);
                console.log(results)
...
function Lose() {
    ...
    location.replace('/smuggling_sim/Ville/End/lose.html?_ijt=2ndqj6kf8liq1g0n0soci913b8&_ij_reload=RELOAD_ON_SAVE')
        ...
        try {
            ...
            for (let i in largeAirports) {
                ...
                function markerCreation() {
                    marker.bindPopup(icao + "<br>" + name + "<br>" + "<button onclick='markerEvent()'>Click to fly here</button>").openPopup;
                }
            }
        } catch (e) {
            console.error(e.message);
...
createShop();
function createShop(action1 = shop1, action2 = shop2, action3 = shop3, action4 = shop4, action5 = shop5) {
    ...
    const p1 = form.appendChild(document.createElement('p')).innerText = 'Jogging shoes for running.(-10% to get caught)';
    ...
    const p2 = form.appendChild(document.createElement('p')).innerText = 'Better packing for less smell and more cover.(-20% to get caught)';
    ...
    const p3 = form.appendChild(document.createElement('p')).innerText = 'Suitcase with secret pocket, to hide products.(-40% to get caught)';
    ...
    const p4 = form.appendChild(document.createElement('p')).innerText = 'Products have better quality.(+20% profit)';
    ...
    const p5 = form.appendChild(document.createElement('p')).innerText = 'Progressive quality control of the products.(20% profit)';
...
function shop1() {
    ...
    async function getJson() {
        try {
            const results = await fetchJson('http://127.0.0.1:3000/shop/' + 'joggers');
            console.log(results)
...
function shop2() {
    ...
    async function getJson() {
        try {
            const results = await fetchJson('http://127.0.0.1:3000/shop/' + 'packing');
            console.log(results)
...
function shop3() {
    ...
    async function getJson() {
        try {
            const results = await fetchJson('http://127.0.0.1:3000/shop/' + 'suitcase');
            console.log(results)
...
function shop4() {
    ...
    async function getJson() {
        try {
            const results = await fetchJson('http://127.0.0.1:3000/shop/' + 'quality');
            console.log(results)
...
function shop5() {
    ...
    async function getJson() {
        try {
            const results = await fetchJson('http://127.0.0.1:3000/shop/' + 'qualityc');
            console.log(results)
